<?php
require 'db.php';
$stmt = $pdo->query("SELECT n.*, c.name AS category_name FROM news_articles n JOIN categories c ON n.category_id = c.id ORDER BY n.published_at DESC LIMIT 4");
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: Arial, sans-serif; }
        ...
        .featured { background: white; padding: 20px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
        .featured h2 { font-size: 1.8em; margin-bottom: 20px; color: #d32f2f; }
    ...
    <nav>
        <ul>
            <?php foreach ($categories as $category): ?>
                <li><a onclick="redirectToCategory(<?php echo $category['id']; ?>)"><?php echo htmlspecialchars($category['name']); ?></a></li>
            <?php endforeach; ?>
        </ul>
    </nav>
    <div class="container">
